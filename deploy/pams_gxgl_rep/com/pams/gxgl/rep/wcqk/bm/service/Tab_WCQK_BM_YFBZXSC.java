...
@Component
@Transactional
public class Tab_WCQK_BM_YFBZXSC
{
	JdbcTemplate jt;

	public List execute(DynamicObject obj) throws Exception
	{
		System.out.println("计划执行时长、实际执行时长-------->>");
		...
		sql.append("select info.deptid, bv.cno,bv.jhsc jhsc, (bv.jhsc - (UF_Calculate_Duration(info.publishtime, info.obtaintime))) cs,(UF_Calculate_Duration(info.publishtime, info.obtaintime)) sjsc").append("\n");
