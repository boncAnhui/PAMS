...
public class ZXQKHelper {
	...
	public static String sql_xxgx_kpi_jsl(DynamicObject obj) throws Exception {
		...
		sql.append(
				" select usr.loginname, usr.cname, usr.ownerdept, usr.deptname, sum(bzsc) bzsc, sum(zxsc) zxsc, sum(cskh)cskh ")
				.append("\n");
		sql.append("   from t_app_infoshare info, t_sys_user usr, ").append(
				"\n");
		sql.append("   (  ").append("\n");
		sql.append(
				"     select runflowkey, cno,  bzsc, zxsc, (case when (zxsc - bzsc) > 5 then 5 else (zxsc - bzsc) end) cskh ")
				.append("\n");
		sql.append("       from  ").append("\n");
		sql.append("       ( ").append("\n");
		...
		// sql.append("              ((case when info.publishtime is null then "
		// + sql_cdate +
		// " - info.obtaintime else info.publishtime - info.obtaintime end)) zxsc ").append("\n");
		if ("Y".equals(ispublish)) {
			sql.append(" (ract.completetime - ract.createtime) zxsc ").append(
					"\n");
		} else if ("N".equals(ispublish)) {
			sql.append(
					" (case when ract.completetime is null then "
							+ sql_cdate
							+ " - info.obtaintime else info.publishtime - info.obtaintime end) zxsc ")
					.append("\n");
		} else {
			sql.append(
					" (case when ract.completetime is null then "
							+ sql_cdate
							+ " - ract.createtime else info.publishtime - info.obtaintime end) zxsc ")
					.append("\n");
		}
		...
		sql.append("           ( ").append("\n");
		sql.append(
				"             select runflowkey, flowdefid, cno, sum(bzsc) bzsc ")
				.append("\n");
		sql.append("               from ").append("\n");
		sql.append("               (  ").append("\n");
		sql.append(
				"                 select ract.runflowkey, ract.flowdefid, info.cno, ract.actdefid, sum(kpi.dvalue) bzsc               ")
				.append("\n");
		sql.append(
				"                   from t_sys_wfract ract, t_sys_wfrflow rflow, t_sys_wfbflow bflow, t_sys_wfbact bact, t_app_kpi kpi, t_app_infoshare info  ")
				.append("\n");
		sql.append("                  where 1 = 1   ").append("\n");
		sql.append(
				"                    and rflow.runflowkey = ract.runflowkey  ")
				.append("\n");
		...
		sql.append("                    and bact.ctype <> 'END'  ")
				.append("\n");
		sql.append("                    and ract.flowdefid = bflow.id  ")
				.append("\n");
		sql.append("                    and bflow.classid = 'GXGL'  ").append(
				"\n");
		sql.append("                    and ract.dataid = info.id ").append(
				"\n");
		sql.append("                    and ract.flowdefid = kpi.flowdefid ")
				.append("\n");
		sql.append("                    and ract.actdefid = kpi.actdefid ")
				.append("\n");
		sql.append("                    and kpi.pname = 'XXGX.ZXSC' ").append(
				"\n");
		if (!StringToolKit.isBlank(begindate)) {
			sql.append(RepHelper.date_begin_eq("rflow.createtime", begindate))
					.append("\n");
		}
		...
		sql.append(
				"                  group by ract.runflowkey, info.cno, ract.flowdefid, ract.actdefid  ")
				.append("\n");
		...
		sql.append(
				"   group by usr.loginname, usr.cname, usr.ownerdept, usr.deptname ")
				.append("\n");
