...
$('#gAjaxTree li span').live('click',function()
{
    ...
    if(obj.find('ul').length==0)
    {
		...
		{
			//如果不是终点 才 ajax
			obj.append('<ul>'+ajax4Tree_pz1('${base}/module/pams/gxgl/zswh/knowledge_treechild.action',obj.attr('data-id'))+'</ul>')
		}
    }else{
	...
	window.rightIframe.location='${base}/module/pams/gxgl/zswh/knowledge_browse.action?cclassid='+obj.attr('data-id')+"&leaf="+obj.attr('data-leaf');
...
<ul id="gAjaxTree">
	<li data-pid="${arg.rootid}" data-index="1" data-parent="" data-leaf="1" data-id="${arg.rootid}"><span class="root">${arg.rootname}</span></li>
