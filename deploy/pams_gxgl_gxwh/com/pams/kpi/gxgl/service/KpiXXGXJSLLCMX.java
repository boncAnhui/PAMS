...
public class KpiXXGXJSLLCMX
{
	...
	public List execute(DynamicObject obj) throws Exception
	{
		...
		sql.append("     select runflowkey, cno, obtaintime, publishtime, bzsc, zxsc, (case when (zxsc - bzsc) > 5 then 5 else (zxsc - bzsc) end) cskh ").append("\n");
		sql.append("       from  ").append("\n");
		sql.append("       ( ").append("\n");
		sql.append("         select zx.runflowkey, zx.cno, info.obtaintime, info.publishtime, zx.bzsc, ").append("\n");
		...
		sql.append("             and info.creater = " + SQLParser.charValue(loginname));
		sql.append("       ) zx     ").append("\n");
		sql.append("       where 1 = 1  ").append("\n");
