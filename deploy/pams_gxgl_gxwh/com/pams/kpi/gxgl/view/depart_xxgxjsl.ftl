...
<h2><span class="title rep_xxgxjslbm">执行时长(部门) 图</span><span class="more"><@pub_macros.showspan chartname = "pki.depart.zxsc" pname="pki_depart_zxsc" /></span></h2>
<div class="p8" id="rep_xxgxjslbm">
</div>
</div>
...
{
	page_load_table("${base}/module/pams/kpi/gxgl/kpi_depart_xxgxjsl_tb.action", "xxgxjslbm", 0, new Array("source","sourcedept"), new Array($("#source").val(),$("#sourcedept").val()));
}

function page_load_dhrx_chart()
{
	page_load_chart("gxgl.pki.xxgxjsl.xxgxjslbm", "xxgxjslbm", 0, "Column3D", new Array("source","sourcedept"), new Array($("#source").val(),$("#sourcedept").val()));
}
