...
$('#file_upload').uploadify({
    ...
    'script'    : '${base}/module/pams/hbgl/file/fileattachment_upload.action',
	...
	'onComplete':function(e,ID, fileObj, response, data){
		if(response!='error'){
			...
			$.each(uploadQueue,function(j,k){				
				ohtml+='<li data-id="'+k[1]+'" data-name="'+k[2]+'"><a target="_blank" class="attachment" href="${base}/module/pams/hbgl/file/fileattachment_downloadbyid.action?id='+k[1]+'">'+uploadNames[j]+'</a><span class="del" title="删除">x</span></li>';
				oid.push(k[1])
				oname.push(k[2])
			})
			ohtml+='</ul>';
...
$('.attachmentUl .del').live('click',function(){
	...
	if(window.confirm('你确认要删除这个附件吗？')){
		...
		$.ajax({
			url:'${base}/module/pams/hbgl/file/fileattachment_isdelete.action',
			data:{attachid:oid},
			success:function(d){
				if(d=='done'){//后台确认已经删除	
				    var new_uploadQueue=[];
	...
	<table class="formGrid">
		...
		<tr>
		    ...
		    <td colspan="3">
				...
				<a target="_blank" class="attachment" href="${base}/module/pams/hbgl/file/fileattachment_downloadbyid.action?attachid=${aobj.attachid}">${aobj.attachname}</a><span class="del">X</span><br>
				</li>
				</#list>
				-->
			</td>     
