...
var navigationJSON=[ {name:'户表管理',link:'${base}/module/irm/portal/portal/portal/portal_browse.action?ccate=hbgl'}, {name:'户表管理'}];
...
$('.topTr li').click(function(){
	...
	// 点击标签页
	if(oindex==1){ // 基本信息
		oFrame.attr('src','apply_readpage.action?runactkey=${arg.runactkey}');
	}else if(oindex==2){ // 过程文档
		oFrame.attr('src','apply_browsefile.action?runactkey=${arg.runactkey}');
	}else if(oindex==3){ // 审批意见
		oFrame.attr('src','about:blank');
	}else if(oindex==4){ // 流程跟踪
		oFrame.attr('src','${base}/module/app/system/workflow/ui/flowtrace.action?runflowkey=${arg.runflowkey}');
	}else if(oindex==5){ // 流程指标
		oFrame.attr('src','${base}/module/app/system/workflow/ui/flowstat.action?runflowkey=${arg.runflowkey}');
	}
...
<tr class="bottomTr">
<td>
	<iframe src="apply_readpage.action?id=${arg.id}&runactkey=${arg.runactkey}" frameborder="0"></iframe>
</td>
</tr>
