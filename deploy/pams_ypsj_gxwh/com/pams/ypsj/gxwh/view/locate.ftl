...
<input type="hidden" id="id" name="id" value="${data.marketpower.id}">
<input type="hidden" id="publishtime" name="publishtime" value="${data.marketpower.publishtime}">
...
<table class="formGrid">
	<tr>
		...
		<td>
		<input type="hidden" id="deptid" name="deptid" value="${data.marketpower.deptid}"/>
		<input readonly class="text required" id="deptname" name="deptname" style="width:20em" value="${data.marketpower.deptname}"/>
		</td>
		<td class="r"><label for="positionname">岗位：</label></td>
		<td>
		<input type="text" readonly id="positionname" name="positionname" value="${data.marketpower.positionname}" style="width:20em">
		</td>
	</tr>
	<tr>
		<td class="r"><label for="creatername">共享人：</label></td>
		<td>
		<input type="text" readonly id="creatername" name="creatername" value="${data.marketpower.creatername}" style="width:20em">
		</td>
		<td class="r"><label for="sourcename">信息来源：</label></td>
		<td>
		<span class="selectSpan">
		<input type="hidden" id="sourceid" name="sourceid" value="" >
		<input class="select readonly required" id="selectsourcename" name="selectsourcename"  data-options="${data.sourcename_texts}" data-values="${data.sourcename_values}" data-default="${data.marketpower.sourceid}">
		</span>
		</td>
	</tr>
	<tr>
		...
		<td>
		<input type="hidden" id="obtaintime" name="obtaintime" value="${data.marketpower.obtaintime}"> 
		<input class="date required" id="obtaintimed" name="obtaintimed" style="width:10em" value="${data.marketpower.obtaintimed}"/>
		<input class="time required" id="obtaintimet" name="obtaintimet" style="width:10em" value="${data.marketpower.obtaintimet}"/>
		</td>
		<td class="r"><label for="publishtime">发布时间：</label></td>
		<td>
		<input type="text" readonly id="publishtime" name="publishtime" value="<#if data.marketpower.publishtime!="">${data.marketpower.publishtime?string("yyyy-MM-dd HH:mm")}</#if>" style="width:20em">
		</td>
	</tr>
	<tr>
		<td class="r"><label for="title">信息共享名称：</label></td>
		<td colspan="3"><input class="text required" id="title" name="title" style="width:50em" value="${data.marketpower.title}"/></td>
	</tr>	
	<tr>
		<td class="r"><label for="summary">内容摘要：</label></td>
		<td colspan="3"><textarea class="text" id="summary" name="summary" maxlength="500">${data.marketpower.summary}</textarea></td>
	</tr>
	<tr>
		<td class="r"><label for="cclassname">分类：</label></td>
		<td>
		<input type="hidden" id="cclassid" name="cclassid" value="${data.marketpower.cclassid}">	
		<input class="text required" id="cclassname" name="cclassname" style="width:20em" value="${data.marketpower.cclassname}"/><button id="bt_cclassname" class="btn2">选择</button></td>
		<#--
		<td class="r"><label id="lb_title">共享权限：</label></td>
		<td>
		<span class="selectSpan">
		<input type="hidden" id="shareauthor" name="shareauthor" value="">
		<input class="select readonly required" id="selectshareauthor" data-options="${data.shareauthor_texts}" data-values="${data.shareauthor_values}" data-default="${data.marketpower.shareauthor}">
		</span>
		</td>
		-->
		<td class="r">&nbsp;</td>
	...
	<tr>
		<td class="r"><label for="marketpowerscope">共享范围：</label></td>
		<td colspan="3">
		<input type="hidden" id="marketpowerscopeid" name="marketpowerscopeid" value="${data.marketpower.marketpowerscopeid}">
		<input type="hidden" id="marketpowerscopectype" name="marketpowerscopectype" value="${data.marketpower.marketpowerscopectype}">
		<input type="hidden" id="marketpowerscopeinternal" name="marketpowerscopeinternal" value="${data.marketpower.marketpowerscopeinternal}">
		<input class="text" id="marketpowerscope" name="marketpowerscope" value="${data.marketpower.marketpowerscope}" style="width:45em" />
	...
	<tr>
		...
		<td colspan="3"><textarea class="text" id="memo" name="memo" maxlength="500">${data.marketpower.memo}</textarea></td>
	</tr>	
	<tr>
		...
		<td><input class="text" id="filetype" name="filetype" value="${data.marketpower.filetype}" style="width:20em"/></td>
	</tr>
	<tr>
		<td class="r">文件列表：</td>
	...
	<tr>
		...
		<ul class="attachmentUl" id="attachmentUl">
			...
			<a target="_blank" class="attachment" href="${base}/module/pams/ypsj/wjwh/fileattachment_downloadbyid.action?id=${afile.id}">【${afile.sfilename}】</a>&nbsp;&nbsp;${afile.creatername}&nbsp;于&nbsp;${afile.createtime}&nbsp;在&nbsp; ${afile.actname}&nbsp;业务环节上传。
...
function page_forwardto(index, endactdefid, endactname, ctype)
{
	...
	if(ctype=="END")
	{
		...
		$.ajax({
			type:"POST",
			url:'${base}/module/pams/ypsj/gxwh/apply_ajaxpublish.action',
...
{
	url = "${base}/module/pams/ypsj/gxwh/apply_selectcclassname.action";
	openwin(url,'',pub_width_small,pub_height_small,null,'新增');
}

function page_scope()
{
	url = "${base}/module/pams/ypsj/gxwh/apply_selectscope.action";
	openwinT(url,'',pub_width_large,pub_height_large,null,'');
}

function page_scope_clear()
{
	$("#marketpowerscope").val("");
	$("#marketpowerscopectype").val("");
	$("#marketpowerscopeid").val("");
	$("#marketpowerscopeinternal").val("");
}

function page_scope_reset()
{
	$("#marketpowerscope").val("${data.defsharescope}");
	$("#marketpowerscopectype").val("${data.defsharescopectype}");
	$("#marketpowerscopeid").val("${data.defsharescopeid}");
	$("#marketpowerscopeinternal").val("${data.defsharescopeinternal}");
}
...
$(function()
{
	...
	function page_delattachment(obj)
	{
		if(window.confirm('你确认要删除这个附件吗？')){
			...
			$.ajax({
				url:'${base}/module/pams/ypsj/wjwh/fileattachment_ajaxdelete.action',
				data:{attachid:oid},
				success:function(d){
					if(d=='done'){//后台确认已经删除	
