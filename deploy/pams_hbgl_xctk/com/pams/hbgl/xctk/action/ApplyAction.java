package com.pams.hbgl.xctk.action;
...
public class ApplyAction extends SimpleAction
{
	...
	public String browsegroupsall() throws Exception
	{
		DynamicObject login_token = (DynamicObject) Struts2Utils.getRequest().getSession().getAttribute(GlobalConstants.sys_login_token);
		String loginname = login_token.getFormatAttr(GlobalConstants.sys_login_user);
		QueryActionHelper helper = new QueryActionHelper();
		...
		Search search = (Search) BeanUtils.cloneBean(queryService.findUniqueByOfSearch("searchname", _searchname));
		search.setMysql(meterapplyService.get_browsegroupsall_sql(amap));// 查询全体记录
	...
	// 创建子流程
	public String createsubflow() throws Exception
	{
		String runactkey = Struts2Utils.getRequest().getParameter("runactkey");
		String tableid = "METERAPPLY";

		String dataid = workFlowEngine.getDemandManager().getRAct(runactkey, tableid).getFormatAttr("dataid");
		DynamicObject obj_ract = workFlowEngine.getDemandManager().getRAct(runactkey, tableid);
		String actdefid = workFlowEngine.getDemandManager().getRAct(runactkey, tableid).getFormatAttr("actdefid");
		String flowdefid = workFlowEngine.getDemandManager().getBAct(actdefid).getFormatAttr("subflowid");

		MeterApply meterapply = meterapplyService.locate(dataid);
		...
		String runactkeysub = meterapplyService.createflow(swapFlow);

		arg.put("id", meterapply.getId());
		arg.put("runactkey", runactkeysub);
