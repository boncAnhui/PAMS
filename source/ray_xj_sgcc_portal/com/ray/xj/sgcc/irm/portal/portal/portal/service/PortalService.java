...
public class PortalService
{
	...
	public String getAllToDo_sql(String loginname) throws Exception
	{
		...
		sql.append(" t.username susername, case when t.ttype='创建' then '' else  t.ttype  end staskname, ").append("\n");
		...
		sql.append(" or ( t.userid='" + loginname + "'  ").append("\n");
		sql.append(" and m.state = '编制' )) ").append("\n");
		...
		sql.append(" l.username susername, case l.dname when 'start' then '新建' when 'leader' then '班组长审核' when 'manager' then '业务主管审核' end taskname,").append("\n");//
		sql.append(" '' taked, c.id bussinessid, '建转运' ctype, c.cno cno, c.pname || '【里程碑】' || d.cname title, '' cclass, c.createtime createtime, '' username ").append("\n");
		sql.append(" from t_app_ctooperation c,t_app_ctooperation_stage d,t_app_ctooperation_log l ").append("\n");
		sql.append(" where 1=1 ").append("\n");
		...
		sql.append(" ract.createtime tasktime, bact.cname taskname,   ").append("\n");
		sql.append(" w.sendercname susername, '' staskname, ").append("\n");
		sql.append(" case when ract.state ='待处理' then '未接单' else '已接单' end  taked , bv.id bussinessid, '隐患' ctype, bv.cno cno, bv.htitle title, '' cclass, ract.createtime createtime, u.cname username ").append("\n");
		sql.append(" from t_app_hdanger bv, t_sys_wfbact bact, t_sys_wfrflow rflow, t_sys_user u , t_sys_wfract ract, t_sys_wfractowner ractowner ").append("\n");
		sql.append(" left join t_sys_wfwaitwork w   ").append("\n");
		sql.append(" on ractowner.runactkey = w.runactkey ").append("\n");
		sql.append(" and ractowner.ownerctx = w.receiver ").append("\n");
		sql.append(" where 1=1 ").append("\n");
		...
		sql.append(" from t_app_knowledge bv, t_sys_wfbact bact, t_sys_wfrflow rflow, t_sys_user u , t_sys_wfract ract, t_sys_wfractowner ractowner ").append("\n");
		sql.append(" left join t_sys_wfwaitwork w   ").append("\n");
		sql.append(" on ractowner.runactkey = w.runactkey ").append("\n");
		sql.append(" and ractowner.ownerctx = w.receiver ").append("\n");
		sql.append(" where 1=1 ").append("\n");
